package repotest.app.shared.authorization;
...
@SourceCodeAuthorClass(createdBy = "root", updatedBy = "", versionNumber = "1", comments = "Roles", complexity = Complexity.LOW)
...
public class Roles implements Serializable, CommonEntityInterface, Comparator<Roles> {
    ...
    @OneToMany(cascade = CascadeType.ALL, fetch = FetchType.EAGER, mappedBy = "roles", orphanRemoval = true)
    ...
    public Roles addRoleMenuBridge(RoleMenuBridge _roleMenuBridge) {
        if (this.roleMenuBridge == null) {
            this.roleMenuBridge = new java.util.ArrayList<repotest.app.shared.authorization.RoleMenuBridge>();
        }
    ...
    public boolean equals(Object obj) {
        try {
            repotest.app.shared.authorization.Roles other = (repotest.app.shared.authorization.Roles) obj;
            if (roleId == null) {
                return false;
