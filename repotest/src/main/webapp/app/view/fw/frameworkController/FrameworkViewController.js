Ext.define('Repotest.view.fw.frameworkController.FrameworkViewController', {
    ...
    onConsoleClick: function() {
         var window = Ext.create('Repotest.view.console.ConsoleWindow');
     ...
     createWindow : function (component){
		...
		modifyComponents : function(configData, view){
			...
			for (var index =0 ; index < items.length ; index++) {
				...
				} else if(items[index].xtype == "customdatetimefield" || items[index].xtype == "datefield") {
					
					var res = configData[items[index]['bindable']].replace(' '+Ext.util.Cookies.get('XA_TID'), '');
				...
				}  else if(items[index].xtype == "image" ) {

					var img = items[index];
					img.setSrc(configData[img.bindable]);
				}  else if(items[index].xtype == 'hidden' || items[index].xtype =='displayfield' ||
  	   				   items[index].xtype == 'textfield' || items[index].xtype == 'hiddenfield' || 
  	   				   items[index].xtype == 'textareafield' || items[index].xtype == 'numberfield' ||
  	   				   items[index].xtype == 'combo' ){
					
					items[index].setValue(configData[items[index]['bindable']]);				
				} else if(items[index].xtype == "label"){
