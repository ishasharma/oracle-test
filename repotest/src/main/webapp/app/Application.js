 ...
 */
Ext.define('Repotest.Application', {
	...
	requires : [ "Ext.ux.BoxReorderer", "Ext.ux.CellDragDrop",
			...
			"Ext.ProgressBarWidget", "Ext.ZIndexManager","Repotest.view.databrowsercalendar.DBCalendar" ],
	...
	stores : [ 'Repotest.store.QueryCriteriaWidgetsStore' ],
...
Ext.Ajax.on('requestcomplete', function(conn, response, options, eOpts) {
	...
	if (responseData.response) {
		if (responseData.response.exceptionId == 4005 && !sessionTimeOutFlag) {
			...
			var loginWindow = Ext.create('Repotest.view.login.SessionLogin')
					.center();
